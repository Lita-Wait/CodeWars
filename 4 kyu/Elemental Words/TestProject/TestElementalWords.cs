...
namespace TestProject
{
    ...
    class BasicCoverage
    {
        ...
        {
            Assert.That(ElementalWords.ElementalForms(""), Is.EqualTo(new string[0][]));
        }

        [Test, Description("Works with non-elemental words")]
        public void NonElementalWords()
        {
            var cases = new[] { "z", "27", " ", "   ", "!!!", "apple", "codewars", "foo bar", " a b c " };
            foreach (var word in cases)
            {
                Assert.That(ElementalWords.ElementalForms(word), Is.EqualTo(new string[0][]));
            }
        }
    ...
    class RandomTests
    {
            ...
            static IEnumerable<string[]> FormWords(string word)
            {
                ...
                else
                {
                    for (int i = 0; i < Math.Min(word.Length, 3); i++)
                    {
                        ...
                        if (Preloaded.ELEMENTS.ContainsKey(element))
                        {
                            var current = new string[] { $"{Preloaded.ELEMENTS[element]} ({element})" };
        ...
        public void RandomEnglishElementalWords()
        {
            for (int i = 0; i < 100; i++)
            {
        ...
        public void RandomElementalWordsWithManyForms()
        {
            for (int i = 0; i < 5; i++)
            {
                var wordSB = new System.Text.StringBuilder();
